module.exports = (db) => {
    ...
    db.run(
      ...
      function (err) {
        if (err) {
          ...
          return res
            .status(500)
            .json({ message: "Error enrolling student in course" });
        }
        res
          .status(201)
  ...
  router.delete("/:courseId/unenroll", checkAuth, (req, res) => {
    ...
    courseModel.unEntroll(courseId, userId, (err) => {
      if (err) {
        return res.status(500).json({ message: "Error unenrolling student" });
      }
      res
        .status(200)
        .json({ message: "Student unenrolled from course successfully" });
    });
  });

  // Get ALl courses for a student
  router.get("/student/:userId", checkAuth, async (req, res) => {
    const { userId } = req.params;
    courseModel.findAllByStudentId(userId, (err, coursesStudent) => {
      if (err) {
        return res.status(500).json({ message: "Error fetching courses" });
      }
      res.status(200).json(coursesStudent);
    });
  });
